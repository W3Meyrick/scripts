...
# Task set for Compute Engine API
class ComputeEngineBehavior(TaskSet):
    ...
    def compute_api_test(self):
        ...
        response = self.client.get(compute_api_url, headers=headers)
        if response.status_code != 200:
            response.failure(f"Failed with status code {response.status_code}")
        else:
            response.success()

# Task set for Storage API
class StorageBehavior(TaskSet):
    ...
    def storage_api_test(self):
        ...
        response = self.client.get(storage_api_url, headers=headers)
        if response.status_code != 200:
            response.failure(f"Failed with status code {response.status_code}")
        else:
            response.success()
...
class ComputeEngineUser(HttpUser):
...
def start_locust_multi(num_processes, hatch_rate):
    locust_cmd = f"locust -f {__file__} --master --no-web --clients 1 --hatch-rate {hatch_rate}"
