...
# Function to delete old images for a given project and family
delete_old_images() {
    local project="$1"
    local family="$2"
    ...
    for image in $(echo "$images" | jq -c '.[]'); do
        ...
        if [ "$creation_timestamp" \< "$cutoff_date" ]; then
            ...
            echo "Deleting image $image_name created on $creation_timestamp"
...
# Iterate through each entry in the JSON file and delete old images
for entry in $(jq -c '.[]' "$json_file"); do
    project=$(echo "$entry" | jq -r '.project')
    family=$(echo "$entry" | jq -r '.family')
    delete_old_images "$project" "$family"
done